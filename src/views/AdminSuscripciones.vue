          ...
          <tr v-for="emp in empresasFiltradas" :key="emp.id">
            ...
            <td>
              <div class="action-btns">
                ...
                <button v-if="emp.suscripcion" @click="eliminarSub(emp.suscripcion.id)" class="cs-btn cs-btn-sm cs-btn-ghost" style="color: var(--cs-error);" title="Eliminar Suscripción">
...
async function eliminarSub(subId) {
  if (confirm("¿Estás seguro de eliminar esta suscripción? Esto no elimina la empresa, pero le quitará el plan.")) {
    await subStore.deleteSuscripcion(subId);
  }
}
