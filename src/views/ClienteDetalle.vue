<template>
  <div class="page">
    <div class="header-main">
      ...
      <div class="acciones-header" v-if="!isNuevo" style="display: flex; gap: 10px;">
        ...
        <button v-if="esSuperAdmin" @click="eliminarClienteActual" class="btn-whatsapp" style="background: #ef4444;">
          Eliminar
        </button>
      </div>
    </div>
...
import { getClienteById, saveCliente, deleteCliente, getEmpresaId } from "@/services/data";
...
async function eliminarClienteActual() {
  if (!confirm("¿Seguro que deseas eliminar este cliente? Se perderán sus datos.")) return;
  try {
    await deleteCliente(route.params.id);
    alert("Cliente eliminado");
    router.push("/clientes");
  } catch (e) {
    alert("Error al eliminar. Puede tener facturas o movimientos asociados.");
  }
}
